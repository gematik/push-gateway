...
@TestPropertySource(
  properties =
    ["spring.artemis.embedded.persistent=false", "logging.level.root=INFO", "gatling.enabled=true"]
)
@ActiveProfiles("integrationtest")
class MockJmsConsumerIT(private val jmsTemplate: JmsTemplate) : ShouldSpec() {

  @MockkSpyBean lateinit var firebaseJmsProcessorSpy: MockFirebaseJmsConsumer
  @MockkSpyBean lateinit var apnsJmsProcessorSpy: MockApnsJmsConsumer
  ...
  init {
    ...
    context("plain notifications") {
      context("listening to messages for apns") {
        ...
        should("process plain message for apns only") {
          ...
          await.atMost(2, TimeUnit.SECONDS).untilAsserted {
            verify(exactly = 1) {
              apnsJmsProcessorSpy.processPlainNotification(
                payload = payload,
                message = any<Message>(),
              )
            }
            verify(exactly = 0) {
              firebaseJmsProcessorSpy.processPlainNotification(
                payload = payload,
                message = any<Message>(),
              )
            }
            verify(exactly = 0) {
              apnsJmsProcessorSpy.processEncryptedNotification(
                payload = any<EncryptedNotificationMessage>(),
                message = any<Message>(),
              )
            }
            verify(exactly = 0) {
              firebaseJmsProcessorSpy.processEncryptedNotification(
                payload = any<EncryptedNotificationMessage>(),
                message = any<Message>(),
              )
            }
      ...
      context("listening to messages for apns - quota") {
        val payload =
          PlainNotificationMessage(
            notification =
              PlainNotification(eventId = "eventId", roomId = "roomId", devices = emptySet()),
      ...
      context("listening to messages for firebase - quota") {
        val payload =
          PlainNotificationMessage(
            notification =
              PlainNotification(eventId = "eventId", roomId = "roomId", devices = emptySet()),
